...
void CCallCountDlg::OnBnClickedRefreshBtn()
{
	...
	{
		if(!POWER_CHECK(1501, "콜카운트 이전날짜 조회", TRUE))
			return;
	}
	...
	if(pRs.Execute(&pCmd))
	{
		...
		while(!pRs.IsEOF())
		{
			...
			sInqueryCount = GetMyNumberFormat(nInqueryCount) +  "(" + GetMyNumberFormat(nTotalCount + nInqueryCount) + ")";

			m_List.InsertItem(nItem, ltoa(nItem, buffer, 10));
			m_List.SetItemText(nItem, 1, strBranchName);
			m_List.SetItemText(nItem, 2, GetDashPhoneNumber(strPhone));
			...
			m_List.SetItemText(nItem, 9, GetMyNumberFormat(strTotalDeposit));

			m_List.SetItemText(nItem, 10, GetMyNumberFormat(strTotalCharge));
			m_List.SetItemText(nItem, 11, GetMyNumberFormat(nTotalRider));
			m_List.SetItemText(nItem, 12, GetMyNumberFormat(nAttendRider));
			m_List.SetItemText(nItem, 13, GetMyNumberFormat(nOrderRider));
		...
		{ 
			...
			m_List.SetItemText(nItem, 8, GetMyNumberFormat(nInqueryCountT));
			CString strTempDeposit, strTempCharge;
			strTempDeposit.Format("%lld", nTotalDepositT);
			strTempCharge.Format("%lld", nTotalChargeT);
			m_List.SetItemText(nItem, 9, GetMyNumberFormat(strTempDeposit));
			m_List.SetItemText(nItem, 10, GetMyNumberFormat(strTempCharge));

			m_List.SetItemText(nItem, 11, GetMyNumberFormat(nTotalRiderT));
			m_List.SetItemText(nItem, 12, GetMyNumberFormat(nAttendRiderT));
			m_List.SetItemText(nItem, 13, GetMyNumberFormat(nOrderRiderT));
...
void CCallCountDlg::OnViewExcel()
{
	...
	/* 김과장 작업
	AddSecurityLogEx(GetCurBranchInfo()->nCompanyCode,
...
//	m_List.InsertColumn(2,"전화",LVCFMT_LEFT, 130);
//	m_List.InsertColumn(3,"총건수",LVCFMT_RIGHT, 70);
//	m_List.InsertColumn(4,"완료수",LVCFMT_RIGHT, 70);
//	m_List.InsertColumn(5,"취소수",LVCFMT_RIGHT, 70);
//	m_List.InsertColumn(6,"취소율",LVCFMT_RIGHT, 70);
//	m_List.InsertColumn(7,"기사부족",LVCFMT_RIGHT, 70);
//	m_List.InsertColumn(8,"총금액",LVCFMT_RIGHT, 100);
//	m_List.InsertColumn(9,"총입금",LVCFMT_RIGHT, 100);
//
//	m_List.Populate();
//
//	OnBnClickedRefreshBtn();
...
//
//	CMkRecordset pRs(m_pMkDb);
//	CMkCommand pCmd(m_pMkDb, "select_call_count6");
//	pCmd.AddParameter(typeLong, typeInput, sizeof(int), m_ci.m_nCompanyCode);
//	pCmd.AddParameter(typeBool, typeInput, sizeof(BOOL), m_ba.GetCount() < 2 ? FALSE : TRUE);
//	pCmd.AddParameter(typeDate, typeInput, sizeof(COleDateTime), m_dtFrom);
//	pCmd.AddParameter(typeDate, typeInput, sizeof(COleDateTime), m_dtTo);
...
//		long nCompleteCountT = 0;
//		long nCancelCountT = 0;
//		long nCancelCount7T = 0;
//		long nTotalDepositT = 0;
//		long nTotalChargeT = 0;
//
//		while(!pRs.IsEOF())
//		{
//			CString strBranchName, strPhone;
//			long nTotalCount, nCompleteCount;
//			long nCancelCount, nCancelCount7, nTotalDeposit, nTotalCharge;
//
//			pRs.GetFieldValue("sBranchName", strBranchName);
//			pRs.GetFieldValue("sTel", strPhone);
//			pRs.GetFieldValue("nTotalCount", nTotalCount);
...
//			pRs.GetFieldValue("nTotalCharge", nTotalCharge);
//
//			long nCancelRate = 0;
//			if(nTotalCount > 0)
//				nCancelRate = nCancelCount * 100 / nTotalCount;
//
//
//			m_List.InsertItem(nItem, ltoa(nItem, buffer, 10));
//			m_List.SetItemText(nItem, 1, strBranchName);
...
//			m_List.SetItemText(nItem, 4, ltoa(nCompleteCount, buffer, 10));
//			m_List.SetItemText(nItem, 5, ltoa(nCancelCount, buffer, 10));
//			m_List.SetItemText(nItem, 6, CString(ltoa(nCancelRate, buffer, 10)) + "%");
//			m_List.SetItemText(nItem, 7, ltoa(nCancelCount7, buffer, 10));
//			m_List.SetItemText(nItem, 8, GetMyNumberFormat(nTotalCharge));
//
//			if(IsThisCompany("퀵오케이"))
//				m_List.SetItemText(nItem, 9, GetMyNumberFormat(nTotalCharge * 0.23));
//			else
//				m_List.SetItemText(nItem, 9, GetMyNumberFormat(nTotalDeposit));
//
//			nTotalCountT += nTotalCount;
//			nCompleteCountT += nCompleteCount;
//			nCancelCountT += nCancelCount;
...
//			long nCancelRateT = 0;
//			if(nTotalCountT > 0)
//				nCancelRateT = nCancelCountT * 100 / nTotalCountT;
//
//			m_List.InsertItem(nItem++, "");
//			m_List.InsertItem(nItem, "");
//			m_List.SetItemText(nItem, 1, "합계");
...
//			m_List.SetItemText(nItem, 4, ltoa(nCompleteCountT, buffer, 10));
//			m_List.SetItemText(nItem, 5, ltoa(nCancelCountT, buffer, 10));
//			m_List.SetItemText(nItem, 6, CString(ltoa(nCancelRateT, buffer, 10)) + "%");
//			m_List.SetItemText(nItem, 7, ltoa(nCancelCount7T, buffer, 10));
//			m_List.SetItemText(nItem, 8, GetMyNumberFormat(nTotalChargeT));
//
//			if(IsThisCompany("퀵오케이"))
//				m_List.SetItemText(nItem, 9, GetMyNumberFormat(nTotalChargeT * 0.23));
//			else
//				m_List.SetItemText(nItem, 9, GetMyNumberFormat(nTotalDepositT));
//
//		}
//
//		m_List.Populate();
...
//		return;
//
//	AddSecurityLog(GetCurBranchInfo()->nCompanyCode,
//		701, m_List.GetItemCount());  
//
//
//	CMyExcel::ToExcel(&m_List);
...
//
//
//void CCallCountDlg::OnBnClickedDateBtn()
//{
//}
