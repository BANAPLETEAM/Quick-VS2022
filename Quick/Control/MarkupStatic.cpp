...
void CMarkupStatic::SetMarkupText(LPCSTR lpszMarkup, LPCSTR lpszNormal)
{
	MARKUP_RELEASE(m_pUIElement);

	USES_CONVERSION;
	LPWSTR lpszwMarkup A2W(lpszMarkup);

	m_pUIElement = Parse(lpszwMarkup);
	m_strMarkupText = lpszMarkup;
	m_strNormalText = lpszNormal; 
