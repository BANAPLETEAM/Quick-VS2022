...
IMPLEMENT_DYNAMIC(CGdipButton, CButton)
...
CGdipButton::~CGdipButton()
{
	...
	if(m_pHotImage) delete m_pHotImage;
...
//
//=============================================================================
BOOL CGdipButton::LoadStdImage(UINT id, LPCTSTR pType, HMODULE hInst)
{
	m_pStdImage = new CGdiPlusBitmapResource;
	return m_pStdImage->Load(id, pType, hInst);
}

BOOL CGdipButton::LoadHotImage(UINT id, LPCTSTR pType, HMODULE hInst)
{
	m_pHotImage = new CGdiPlusBitmapResource;
	return m_pHotImage->Load(id, pType, hInst);
}
...
//
//=============================================================================
BOOL CGdipButton::LoadAltImage(UINT id, LPCTSTR pType, HMODULE hInst)
{
		...
		if (m_dcStd.m_hDC == NULL)
		{
			...
			if(m_dcStdH.m_hDC == NULL)
			{
				if(m_pHotImage)
				{
					...
					graphics.DrawImage(*m_pHotImage, 0, 0);
...
void CGdipButton::SetBkGnd(CDC* pDC)
{
	...
	//by mksong (2013-06-12 오후 12:08)
	//배경이 변경되면 버튼을 다시 그리기 위해서
	//아래 코드 추가함
	m_bHaveBitmaps = FALSE;
	m_dcStd.DeleteDC();
}
