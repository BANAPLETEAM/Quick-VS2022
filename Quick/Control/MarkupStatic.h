#if !defined(AFX_MARKUPSTATIC_H__9851CB9F_CEAA_444A_87BB_C508D160702B__INCLUDED_)
#define AFX_MARKUPSTATIC_H__9851CB9F_CEAA_444A_87BB_C508D160702B__INCLUDED_

#if _MSC_VER > 1000
#pragma once
#endif // _MSC_VER > 1000
// MarkupStatic.h : header file
//

/////////////////////////////////////////////////////////////////////////////
// CMarkupStatic window

class CMarkupStatic : public CStatic, public CXTPMarkupContext
{
// Construction
public:
	CMarkupStatic();

// Attributes
public:

// Operations
public:
	void SetMarkupText(LPCSTR lpszMarkup,  LPCSTR lpszNormal = "");
	void GetNormalText(CString &strNormalText) { strNormalText = m_strNormalText; }
	CString GetNormalText() { return m_strNormalText;}
	void GetMarkupText(CString &strMarkupText) { strMarkupText = m_strMarkupText; }
	CString GetMarkupText() { return m_strMarkupText; }

// Implementation
public:
	virtual ~CMarkupStatic();

	// Generated message map functions
protected:
	DECLARE_MESSAGE_MAP()

	//{{AFX_VIRTUAL(CMarkupStatic)
	BOOL OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult);
	//}}AFX_VIRTUAL

	//{{AFX_MSG(CMarkupStatic)
	afx_msg void OnPaint();
	//}}AFX_MSG

	CXTPMarkupUIElement* m_pUIElement;
	CString m_strNormalText;
	CString m_strMarkupText;
};

/////////////////////////////////////////////////////////////////////////////

//{{AFX_INSERT_LOCATION}}
// Microsoft Visual C++ will insert additional declarations immediately before the previous line.

#endif // !defined(AFX_MARKUPSTATIC_H__9851CB9F_CEAA_444A_87BB_C508D160702B__INCLUDED_)
