...
public:
	...
	// bro    2002-02-20
	//===========================================================
	static inline BOOL vmInitPool( DWORD dwPoolSize , BOOL bAllocNow = FALSE, DWORD dwObjSize = 0, BOOL bDeleteOuterBuffer = FALSE)
	{
		ms_dwPoolSize = dwPoolSize;
		ms_bDeleteOuterBuffer = bDeleteOuterBuffer;
...
private:
	...
	static BOOL		ms_bDeleteOuterBuffer;			// 외부할당 버퍼를 에러없이 delete 시킴 //by mksong (2012-06-13 오후 3:34)
...
public:
	...
	void* operator new( size_t size )
	{
		...
		if( !ms_pMemPool )
		{
			//이전과 달리, vminit에서만 초기화되도록한다. 
			//vmInit 실행전에 new들어와서 template 변수로 초기화되는걸 막는다.
			ASSERT(ms_dwPoolSize);
	...
	void operator delete( void* p )
	{
		if(p)
		{
			BOOL bValidPtr = p>=ms_pData&&
							p<=ms_pData+(ms_dwPoolSize*ms_dwObjSize)&&
							((char*)p-ms_pData)%ms_dwObjSize == 0;

			if(!bValidPtr)
			{
				if(ms_bDeleteOuterBuffer)
				{
					free(p);
					return;
				}
...
template <class T> CBitSet CVMemPool<T>::ms_bsAllocSet;
template <class T> DWORD CVMemPool<T>::ms_dwSrtFree;
template <class T> DWORD CVMemPool<T>::ms_dwEndFree;
template <class T> BOOL CVMemPool<T>::ms_bEmptyFree;
template <class T> BOOL CVMemPool<T>::ms_bFullFree;
template <class T> DWORD CVMemPool<T>::ms_dwTotalSize;
template <class T> DWORD CVMemPool<T>::ms_dwFreeQueueSize;
template <class T> char* CVMemPool<T>::ms_pData;

template <class T> typename CVMemPool<T>::CSLock CVMemPool<T>::ms_csLock;
template <class T> char*	CVMemPool<T>::ms_pMemPool = NULL;
template <class T> DWORD CVMemPool<T>::ms_dwObjSize;
template <class T> DWORD CVMemPool<T>::ms_dwAllocObjCount;
template <class T> DWORD CVMemPool<T>::ms_dwPoolSize;

template <class T> BOOL CVMemPool<T>::ms_bDeleteOuterBuffer;
