...
{
	m_hIOCP=NULL;
	m_CreatedThreadNumber = 0;
	InterlockedExchange(&m_nNumOfThread, 0);
	InterlockedExchange(&m_nNumOfActiveThread, 0);
}
...
BOOL IocpHandler::Associate(HANDLE hDevice, ULONG_PTR CompletionKey, int* pnOutErrCode)
{
	...
	HANDLE retHandle=CreateIoCompletionPort(hDevice,m_hIOCP,(DWORD)CompletionKey,0);
...
{
	BOOL ret=PostQueuedCompletionStatus(m_hIOCP,dwBytesTransferred,(DWORD)CompletionKey,pOverlapped);
...
{
	BOOL ret=GetQueuedCompletionStatus(m_hIOCP,pdwOutBytesTransferred, (LPDWORD)pOutCompletionKey,pOutOverlapped,dwWaitingTime);
	if(!ret && pnOutErrCode!=NULL)
	{
       *pnOutErrCode=GetLastError();	   
	}
...
{
	if(m_CreatedThreadNumber == 0)
		return;

	for(int i=0; i< m_CreatedThreadNumber;i++)
	{
//		BANA_LOG("THREAD_DIE\n");
		PostCompletionStatus((ULONG_PTR)THREAD_DIE);
	}

	DWORD dwTick = GetTickCount();

	while(GetNumOfThread() > 0)	
	{
