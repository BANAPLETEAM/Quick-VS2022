						...
						case 1 :
							if(graphType != SCATTER_GRAPH)
							{
								...
								for(int x = 1; x <= graphSeries->GetCount(); x++)
								{
									...
									seriesSpace = (long)(xAxisWidth / graphSeries->GetCount());
									tickXLocation = (long)(xApexPoint + ((x * seriesSpace) - (seriesSpace / 2)));
						...
						case 1 :
							...
							if(graphType != SCATTER_GRAPH)
							{
								...
								for(y = 1; y <= graphSeries->GetCount(); y++)  
								{
									...
									seriesSpace = (long)(yAxisHeight / graphSeries->GetCount());
		...
		case STACKED_BAR_GRAPH_3D :
				...
				if(graphAlignment)
				{
					...
					for(int x = 1; x <= graphSeries->GetCount(); x++)
					{
						...
						{
							CFont xFont;
							xFont.CreateFont(yTickFontSize, 0, 0, 0, 700, FALSE, FALSE, 0,
								ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
								DEFAULT_QUALITY, DEFAULT_PITCH | FF_ROMAN,"Arial");
							pDC->SelectObject(&xFont);
							pDC->TextOut(tickXLocation - ((tickLabel.GetLength() * 8) / 2), yApexPoint + 8, tickLabel);
				...
				else
				{
					...
					//draw y axis ticks
					tickScale = (yAxisHeight - 10) / (seriesSize == 0 ? 1 : seriesSize);
					POSITION pos;
					pos = graphSeries->GetHeadPosition();
					CGraphSeries* tmpSeries;
...
//	int LegendSize = min(seriesSize, m_legendsize);
				...
				else
				{
					...
					if(graphType != SCATTER_GRAPH)
					{
						...
						for(int y = 1; y <= graphSeries->GetCount(); y++)  
						{
							...
							int seriesSpace;
							int tickYLocation;
							seriesSpace = (long)((yAxisHeight - 50) / graphSeries->GetCount());
							tickYLocation = yApexPoint + ((y * seriesSpace) - (seriesSpace / 2));
...
void CGraph::DrawPieSeries(CDC* pDC)
{
	...
	for(int x = 1; x <= graphSeries->GetCount(); x++)
	{
		...
		{
			pieLeft = (long)(xApexPoint + 15 + ((x * 2 - 1) * ((xAxisWidth / graphSeries->GetCount()) / 2)) - radius);
		}
...
void CGraph::DrawBoxWhiskerSeries(CDC* pDC)
{
	...
	for(int x = 1; x <= graphSeries->GetCount(); x++)
	{
		...
		{
			//size is not 0 base, so below, I subtract 1 to 0 base it.
			medialElement = (long)(sortedArray.GetSize() / 2);
			median = sortedArray.GetAt(medialElement);
			highValue = sortedArray.GetAt(sortedArray.GetSize() - 1);
			lowValue = sortedArray.GetAt(0);
			div_t div2Result;
...
//	{
		for(int s = 0; s < seriesSize; s++)
		{
			...
			for(int x = 1; x <= graphSeries->GetCount(); x++)
			{
				...
				if(tmpSeries->GetData(s) > -1)
				{	
					long nMissPoint = (int)((xAxisWidth / graphSeries->GetCount() + 1) / 2) + x;
					tickXLocation = xApexPoint + (int)(x * (xAxisWidth / graphSeries->GetCount() + 1) - nMissPoint);
					...
					pOldPen = pDC->SelectObject(&linePen);
					//front side of line
					long a = lastXLoc + ((seriesSize - s) * (depth / seriesSize));
					long b = lastYLoc - ((seriesSize - s) * (depth / seriesSize));
					long c = tickXLocation + ((seriesSize - s) * (depth / seriesSize));
					long d = yDataLocation - ((seriesSize - s) * (depth / seriesSize));
					...
					else
					{
						...
						{
							CPen bottomPen(PS_SOLID, 1, barColor);
						...
						{
							CPen topPen(PS_SOLID, 1, barColor);
							CBrush topBrush(topColor);
							pDC->SelectObject(&topPen);
							pDC->SelectObject(&topBrush);
...
void CGraph::Print3DBarSeries(CDC* pDC)
{
	...
	{
		//determine width of barchart data blocks
		seriesSpace = (long)((xAxisWidth - 100) / graphSeries->GetCount());
		barWidth = (int)((seriesSpace * .8) / seriesSize);
		dataPlotSize = seriesSize * barWidth;
...
{
	POSITION pos;
	CGraphDataColor* dataColor = nullptr;
	pos = colorList->GetHeadPosition();
	for(int i = 0; i < dataGroup; i++)
	{
		dataColor = (CGraphDataColor*)colorList->GetNext(pos);
