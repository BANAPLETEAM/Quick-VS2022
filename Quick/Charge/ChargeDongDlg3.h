...
#define DONG_POS_LI		20
#define DONG_POS_GU		10
#define DONG_POS_DONG	0
...
class CDetailChargeNewRecord : public CXTPGridRecord
{
protected:
...
public:
	CDetailChargeNewRecord(long nID,BOOL bHasNotDong, int nClass, 
		CString sArea, CString sDong, BOOL bExcel, long nDongID)
	{
		m_bExcel = bExcel;
		m_nID = nID;
	...
	int GetTypeInfo()
	{
		if(m_nClass == DONG_POS_GU && m_bHasNotDong)
			return 3;
		else if(m_nClass == DONG_POS_GU && m_bHasNotDong == FALSE)
			return 2;
		else if(m_nClass == DONG_POS_DONG || m_nClass == 3 )
			return 1;
		else if(m_nClass == DONG_POS_LI)
			return -1;
		else
			return 0;
	}
...
class CDetailDestChargeNewRecord : public CXTPGridRecord
{
protected:
...
public:
	CDetailDestChargeNewRecord(long nID,BOOL bHasNotDong, int nClass, CString sArea,   
		CString sDong, 
		CString sGeneralMotoCharge, CString sGeneralDaMaCharge, CString sGeneralRaboCharge, 
		CString sCompanyMotoCharge,	CString sCompanyDaMaCharge, CString sSubWayCharge, 
		BOOL bExcel, long nDongID)
	{
		m_nID = nID;
		m_bHasNotDong = bHasNotDong;
		m_nClass = nClass;
...
class CMyDestNewReport : public CXTPGridControl
{
public:
	CMyDestNewReport()
	{
...
public:
	...
	void SetChargeDong(CChargeDongDlg3 *pDongDlg){m_pChargeDongDlg3 = pDongDlg;};
};

typedef map<long,CDetailDestChargeNewRecord*> DEST_NEW_RECORD_MAP;
typedef vector<CDetailDestChargeNewRecord*> DEST_NEW_RECORD_VEC;


class CChargeDongDlg3 : public CXTResizeDialog
{
	DECLARE_DYNAMIC(CChargeDongDlg3)

public:
	CChargeDongDlg3(CWnd* pParent = NULL);   // 표준 생성자입니다.
	virtual ~CChargeDongDlg3();

// 대화 상자 데이터입니다.
	enum { IDD = IDD_CHARGE_DONG_DLG3 };
...
public:
	...
	int GetType(CDetailChargeNewRecord *pStartRecord, CDetailDestChargeNewRecord *pDestRecord);
	...
	CMyDestNewReport m_lstDest;
	
	DEST_NEW_RECORD_MAP m_pDestRecord;
	DEST_NEW_RECORD_VEC m_vecDest;
	...
	int m_nFirstWork;
	CChargeDongDlg3 *m_CopyDongDlg3;
