...
// CChargeDongDlg3 대화 상자입니다.

IMPLEMENT_DYNAMIC(CChargeDongDlg3, CXTResizeDialog)
CChargeDongDlg3::CChargeDongDlg3(CWnd* pParent /*=NULL*/)
	: CXTResizeDialog(CChargeDongDlg3::IDD, pParent)
{
...
CChargeDongDlg3::~CChargeDongDlg3()
{
	...
	if(m_CopyDongDlg3 != NULL)
	{
		delete m_CopyDongDlg3;
		m_CopyDongDlg3 = NULL;
	}
...
void CChargeDongDlg3::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CChargeDongDlg3, CXTResizeDialog)
	...
	ON_BN_CLICKED(IDC_AREA_SELECT_BTN, OnBnClickedAreaSelectBtn)
	...
	ON_WM_PAINT()
	ON_BN_CLICKED(IDC_TWOWAY_CHECK, &CChargeDongDlg3::OnBnClickedTwowayCheck)
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_GENERAL_RABO_CHECK, &CChargeDongDlg3::OnBnClickedGeneralRaboCheck)
END_MESSAGE_MAP()


// CChargeDongDlg3 메시지 처리기입니다.



BOOL CChargeDongDlg3::OnInitDialog()
{
	...
	CXTPGridColumn* pCol5 = m_lstDest.AddColumn(new CXTPGridColumn(5, _T("승용"), 55,FALSE));//기존 밴 요금
...
void CChargeDongDlg3::CopyDongDlg()
{
	if(m_CopyDongDlg3 == NULL)
	{
		m_CopyDongDlg3 = new CChargeDongDlg3(this);
		
	}

	m_CopyDongDlg3->m_nCompany = m_nCompany;
	m_CopyDongDlg3->m_bIntegrated = m_bIntegrated;
	m_CopyDongDlg3->m_bCopyDoc = TRUE;
	m_CopyDongDlg3->Create(IDD_CHARGE_DONG_DLG3,this);		
	m_CopyDongDlg3->ShowWindow(SW_SHOW);
	

}

void CChargeDongDlg3::ChargeTypeNameRefresh()
{
...
void CChargeDongDlg3::InitControl(void)
{	
	...
	if(m_ba.GetCount() > 1)
	{
		...
		if(nCurSel >= 0)
		{
			pBi = (CBranchInfo*)m_cmbCompany.GetItemData(nCurSel);			
	...
	CXTPGridRecord *pDestFifthMainRecord = NULL;
	
	CMkRecordset pRs2(m_pMkDb);
	
	//CMkCommand pCmd2(m_pMkDb, "select_dong_pos_all_1");	 
	CMkCommand pCmd2(m_pMkDb, "select_dong_pos_all2015_1");	 
	pCmd2.AddParameter(typeLong, typeInput, sizeof(long), m_nCompany);
	pCmd2.AddParameter(typeLong, typeInput, sizeof(long), m_ci.m_bUseDetailDong);
	pCmd2.AddParameter(typeLong, typeInput, sizeof(long), pBi->bDongPosLiUse);		
	...
	CString sSido, sGugun, sGugun2,sSumGugun, sDong, sPreSido = "",sPreGugun = "", 
			sPreGugun2 = "", sPreSumGugun = "", sPreDong = "", sLi = "", sPreLi ="";
	if(pRs2.Execute(&pCmd2))
	{
		...
		{	
			...
			if(sPreSido != sSido)
			{					
				pStartMainRecord = m_lstStart.AddRecord(new CDetailChargeNewRecord(0, bHasNotDong, nClass, sSido , "지역구분", FALSE, nID));				
				pDestMainRecord = m_lstDest.AddRecord(new CDetailDestChargeNewRecord(0, bHasNotDong, nClass, sSido , "지역구분","","", "", "","", "", TRUE, nID));				
			...
			{
				pStartSubMainRecord = pStartMainRecord->GetChilds()->Add(new CDetailChargeNewRecord	( nID, bHasNotDong, nClass,  sSumGugun , sDong, TRUE, nID));
				pDestSubMainRecord = pDestMainRecord->GetChilds()->Add(new CDetailDestChargeNewRecord(nID, bHasNotDong, nClass,  sSumGugun , sDong, "","", "", "","","", TRUE, nID));					

				m_pDestRecord.insert(DEST_NEW_RECORD_MAP::value_type(nID,(CDetailDestChargeNewRecord*)pDestSubMainRecord));
			}
			...
			{				
				pStartthridMainRecord = pStartSubMainRecord->GetChilds()->Add(new CDetailChargeNewRecord(  nID, bHasNotDong, nClass,  sSumGugun + " " , sDong, TRUE, nID));
				pDestthridMainRecord = pDestSubMainRecord->GetChilds()->Add(new CDetailDestChargeNewRecord(nID, bHasNotDong, nClass,  sSumGugun + " " , sDong,"","","","","","", FALSE, nID));				
				m_pDestRecord.insert(DEST_NEW_RECORD_MAP::value_type(nID,(CDetailDestChargeNewRecord*)pDestthridMainRecord));
			}
			else if(
						(sPreSido == sSido &&
			...
			{
				pStartFourthMainRecord = pStartthridMainRecord->GetChilds()->Add(new CDetailChargeNewRecord(  nID, bHasNotDong, nClass, nClass == DONG_POS_LI ? sDong : "" , nClass == DONG_POS_LI ? sLi : sDong, TRUE,nID));
				pDestFourthMainRecord = pDestthridMainRecord->GetChilds()->Add(new CDetailDestChargeNewRecord(nID, bHasNotDong, nClass, nClass == DONG_POS_LI ? sDong : "" , nClass == DONG_POS_LI ? sLi : sDong, "","","","","","", FALSE, nID));				 
				m_pDestRecord.insert(DEST_NEW_RECORD_MAP::value_type(nID,(CDetailDestChargeNewRecord*)pDestFourthMainRecord));
			}
			else
			{
				if(pBi->bDongPosLiUse && nClass == DONG_POS_LI)
				{
					pStartFourthMainRecord->GetChilds()->Add(new CDetailChargeNewRecord(nID, bHasNotDong, nClass, sDong, sLi,TRUE, nID));
					pDestFifthMainRecord = pDestFourthMainRecord->GetChilds()->Add(new CDetailDestChargeNewRecord(nID, bHasNotDong, nClass, sDong, sLi,  "","","","","","", FALSE, nID));				 
					m_pDestRecord.insert(DEST_NEW_RECORD_MAP::value_type(nID,(CDetailDestChargeNewRecord*)pDestFifthMainRecord));
				}
...
BOOL CChargeDongDlg3::CheckAreaSelect()
{
	...
	CDetailChargeNewRecord *pStartRecord = NULL;
	for(int i = 0; i < nStartCurRow; i++)
	{
		pStartRecord = (CDetailChargeNewRecord*)m_lstStart.GetSelectedRows()->GetAt(i);

		CXTPGridRow *pRow = m_lstStart.GetSelectedRows()->GetAt(i);
		pStartRecord = (CDetailChargeNewRecord*)pRow->GetRecord();
		...
		for(int j=0; j < nDestCurRow; j++)
		{
			...
			pDestRecord =  (CDetailDestChargeNewRecord*)pRow->GetRecord();
...
BOOL CChargeDongDlg3::IsUpdateOk()
{
	CBranchInfo *p = (CBranchInfo*)m_cmbCompany.GetItemData(m_cmbCompany.GetCurSel());
	...
	{		
		...
		for(int i = 0; i < nStartCurRow; i++)
		{
			...
			for(int j=0; j < nDestCurRow; j++)
			{
				...
				CMkCommand pCmd(m_pMkDb, "update_charge_dong_info_q4");
...
int CChargeDongDlg3::GetType(CDetailChargeNewRecord *pStartRecord, CDetailDestChargeNewRecord *pDestRecord)
{
...
void CChargeDongDlg3::OnViewListPrint()
{

	CDetailChargeNewRecord *pReocrd = (CDetailChargeNewRecord *)m_lstStart.GetSelectedRows()->GetAt(0)->GetRecord();
...
void CChargeDongDlg3::OnReportStartItemRClick(NMHDR * pNotifyStruct, LRESULT * /*result*/)
{
...
void CChargeDongDlg3::OnReportBeginEdit(NMHDR * pNotifyStruct, LRESULT * /*result*/)
{
	XTP_NM_REPORTRECORDITEM* pItemNotify = (XTP_NM_REPORTRECORDITEM*)pNotifyStruct;
...
void CChargeDongDlg3::OnReportDestItemRClick(NMHDR * pNotifyStruct, LRESULT * /*result*/)
{
	XTP_NM_REPORTRECORDITEM* pItemNotify = (XTP_NM_REPORTRECORDITEM*) pNotifyStruct;
...
void CChargeDongDlg3::OnReportStartItemClick(NMHDR * pNotifyStruct, LRESULT * /*result*/)
{
	XTP_NM_REPORTRECORDITEM* pItemNotify = (XTP_NM_REPORTRECORDITEM*) pNotifyStruct;
	...
	CDetailChargeNewRecord *pRecord = (CDetailChargeNewRecord *)pItemNotify->pRow->GetRecord();
	if(pRecord->m_nID == 0)
		return;
...
void CChargeDongDlg3::OnReportDestItemClick(NMHDR * pNotifyStruct, LRESULT * /*result*/)
{
	XTP_NM_REPORTRECORDITEM* pItemNotify = (XTP_NM_REPORTRECORDITEM*) pNotifyStruct;
	...
	CDetailDestChargeNewRecord *pRecord = (CDetailDestChargeNewRecord*)pItemNotify->pRow->GetRecord();
...
void CChargeDongDlg3::OnBnClickedDeleteBtn()
{
	...
	CDetailChargeNewRecord *pStartRecord = NULL;
	BOOL bSuccess = FALSE;
	DEST_NEW_RECORD_MAP::iterator it;
	...
	for(int i = 0; i < nStartCurRow; i++)
	{
		...
		for(int j=0; j < nDestCurRow; j++)
		{
			...
			CMkCommand pCmd(m_pMkDb, "delete_charge_dong_info_q4");
			pCmd.AddParameter(typeLong, typeInput, sizeof(long), m_nCompany);					
			pCmd.AddParameter(typeLong, typeInput, sizeof(int), pStartRecord->m_nID);
			pCmd.AddParameter(typeLong, typeInput, sizeof(int), pDestRecord->m_nID);
			pCmd.AddParameter(typeBool, typeInput, sizeof(BOOL), m_chkShuttle.GetCheck());
			...
			pDestRecord = (CDetailDestChargeNewRecord *)it->second;
			if(m_chkGeneralDama.GetCheck())
...
void CChargeDongDlg3::SetAllCharge(long nID, long nCharge)
{
	char buffer[10];
	DEST_NEW_RECORD_MAP::iterator it;
	it = m_pDestRecord.find(nID);
	CDetailDestChargeNewRecord * pDestRecord = (CDetailDestChargeNewRecord *)it->second;	
...
void CChargeDongDlg3::OnSize(UINT nType, int cx, int cy)
{
	CXTResizeDialog::OnSize(nType, cx, cy);
}
void CChargeDongDlg3::ChargeAllDelete()
{
	DEST_NEW_RECORD_MAP::iterator it;
...
void CChargeDongDlg3::SetCharge(long nDestID, int nCol, long nCharge)
{
	char buffer[10];
	DEST_NEW_RECORD_MAP::iterator it;
...
void CChargeDongDlg3::SetAllCharge(long nDestID, long nMotoCharge, long nDamaCharge, long nRaboCharge,
...
void CChargeDongDlg3::OnCbnSelchangeDisComCombo()
{
	RefreshList();
}

void CChargeDongDlg3::RefreshList()
{
	g_bana_log->Print("RefreshList() 시작 %d분:%d초",COleDateTime( COleDateTime::GetCurrentTime() ).GetMinute() , 
					COleDateTime( COleDateTime::GetCurrentTime() ).GetSecond() );
	ChargeAllDelete();
	...
	CDetailChargeNewRecord *pRecord = (CDetailChargeNewRecord *)pRow->GetRecord();
	...
	CMkRecordset pRs(m_pMkDb);
	CMkCommand pCmd(m_pMkDb, "select_charge_dong_total_q3");
	//CMkCommand pCmd(m_pMkDb, "select_charge_dong_total2012_1");
	...
	if(pRs.Execute(&pCmd))
	{
		...
		g_bana_log->Print("Execute() 시작 %d분:%d초",COleDateTime( COleDateTime::GetCurrentTime() ).GetMinute() , 
			COleDateTime( COleDateTime::GetCurrentTime() ).GetSecond() );
	...
	g_bana_log->Print("Populate() 전 %d분:%d초",COleDateTime( COleDateTime::GetCurrentTime() ).GetMinute() , 
		COleDateTime( COleDateTime::GetCurrentTime() ).GetSecond() );
	...
	g_bana_log->Print("Populate() 후 %d분:%d초",COleDateTime( COleDateTime::GetCurrentTime() ).GetMinute() , 
		COleDateTime( COleDateTime::GetCurrentTime() ).GetSecond() );
}

void CChargeDongDlg3::OnBnClickedAllDelCheck()
{
...
void CChargeDongDlg3::OnBnClickedLineBtn()
{
	CLineDlg dlg;
	dlg.m_nCompany = m_nCompany;
...
void CChargeDongDlg3::OnBnClickedAreaSelectBtn()
{
		
	CREATE_MODALESS(CChargeSimpleListDlg, this);
	pDlg->m_nCompany = m_nCompany;	
	pDlg->m_bChargeDongDlg3 = TRUE;
...
void CChargeDongDlg3::OnCbnSelchangeCompanyCombo()
{
	if(m_cmbDiscountCompany.GetCurSel() > 0)
		m_ChargeTypeCmb.SetCurSel(0);
...
void CChargeDongDlg3::OnCbnSelchangeChargeTypeCombo()
{
	if(m_ChargeTypeCmb.GetCurSel() > 0)
		m_cmbDiscountCompany.SetCurSel(0);
...
void CChargeDongDlg3::OnContextMenu(CWnd* pWnd, CPoint point)
{
	...
	pStartRecord = (CDetailChargeNewRecord*)m_lstStart.GetSelectedRows()->GetAt(0)->GetRecord();
	pDestRecord = (CDetailDestChargeNewRecord*)m_lstDest.GetSelectedRows()->GetAt(0)->GetRecord();
...
void CChargeDongDlg3::OnViewExcel()
{
	...
	CDetailChargeNewRecord *pStartRecord = (CDetailChargeNewRecord *)pStartRow->GetRecord();
	CDetailDestChargeNewRecord *pDestRecord = (CDetailDestChargeNewRecord *)pDestRow->GetRecord();
...
void CChargeDongDlg3::OnBnClickedSearchGroupBtn()
{
	CSearchGroupDlg dlg;
...
void CChargeDongDlg3::OnBnClickedSameCheckBtn()
{
	...
	{
		CDetailChargeNewRecord *pRecord = (CDetailChargeNewRecord *)m_lstStart.GetSelectedRows()->GetAt(i)->GetRecord();
		...
		for(int i = 0; i<  m_lstDest.GetRows()->GetCount(); i++)
		{
			...
			long nDestDongID = ((CDetailChargeNewRecord*)pRow->GetRecord())->m_nDongID;
	...
	CString sViewText;
	sViewText.Format("L:%d - R:%d", m_lstStart.GetSelectedRows()->GetCount(), nSameCount);
	m_btnSameView.SetWindowText(sViewText);
}

void CChargeDongDlg3::OnBnClickedButton2()
{
...
void CChargeDongDlg3::OnBnClickedSameViewBtn()
{

}

void CChargeDongDlg3::OnBnClickedChargeListBtn()
{
	CChargeListSetDlg dlg;
	dlg.DoModal();
}

void CChargeDongDlg3::OnBnClickedSaveBtn()
{
	SaveCharge(TRUE);	
}

void CChargeDongDlg3::OnReportValueChanged(NMHDR*  pNotifyStruct, LRESULT* /*result*/)
{
	XTP_NM_REPORTRECORDITEM* pItemNotify = (XTP_NM_REPORTRECORDITEM*) pNotifyStruct;
	...
	try
	{
		...
		pDestRecord = (CDetailDestChargeNewRecord*)pItemNotify->pRow->GetRecord();		
		...
		for(int i = 0; i < m_lstStart.GetSelectedRows()->GetCount(); i++)
		{			
			pStartRecord = (CDetailChargeNewRecord *)m_lstStart.GetSelectedRows()->GetAt(i)->GetRecord();
			...
			{ 
				...
				CMkCommand pCmd(m_pMkDb, "update_charge_dong_info_one_q4");
...
void CChargeDongDlg3::SaveCharge(BOOL bClick)
{
	...
	CDetailChargeNewRecord *pStartReport = (CDetailChargeNewRecord*)pRow->GetRecord();
	...
	{
		CDetailDestChargeNewRecord * pDestReport = m_vecDest.at(i);
		...
		{ 
			...
			CMkCommand pCmd(m_pMkDb, "update_charge_dong_real_q4");
...
void CChargeDongDlg3::OnLButtonUp(UINT nFlags, CPoint point)
{
	CXTResizeDialog::OnLButtonUp(nFlags, point);
}

void CChargeDongDlg3::OnBnClickedSearchSaveBtn()
{
...
void CChargeDongDlg3::OnBnClickedUserBtn()
{
CChargeDongUDlg dlg;
dlg.DoModal();
}

BOOL CChargeDongDlg3::PreTranslateMessage(MSG* pMsg)
{
	return CXTResizeDialog::PreTranslateMessage(pMsg);
}


void CChargeDongDlg3::FocusUp(CXTPGridControl *pControl)
{
...
void CChargeDongDlg3::FocusDown(CXTPGridControl *pControl)
{
	POSITION pos = pControl->GetSelectedRows()->GetFirstSelectedRowPosition();
	int index = pControl->GetSelectedRows()->GetNextSelectedRow(pos)->GetIndex();
...
void CChargeDongDlg3::FocusLeft(CXTPGridControl *pControl)
{
	POSITION pos = pControl->GetSelectedRows()->GetFirstSelectedRowPosition();
	int index = pControl->GetSelectedRows()->GetNextSelectedRow(pos)->GetIndex();
...
void CChargeDongDlg3::FocusRight(CXTPGridControl *pControl)
{
	POSITION pos = pControl->GetSelectedRows()->GetFirstSelectedRowPosition();
	int index = pControl->GetSelectedRows()->GetNextSelectedRow(pos)->GetIndex();
...
BOOL CMyDestNewReport::PreTranslateMessage(MSG* pMsg)
{
	if(pMsg->message == WM_KEYDOWN)
	{
		if(pMsg->wParam == VK_UP)
		{
			m_pChargeDongDlg3->FocusUp(this);
		}
		else if(pMsg->wParam == VK_DOWN)
		{
			m_pChargeDongDlg3->FocusDown(this);
		}
		else if(pMsg->wParam == VK_LEFT)
		{
			m_pChargeDongDlg3->FocusLeft(this);
		}
		else if(pMsg->wParam == VK_RIGHT)
		{
			m_pChargeDongDlg3->FocusRight(this);
		}
	}
...
void  CChargeDongDlg3::SetHTypeViewMode(BOOL bTRUE, long nMemCNo, CString sMemName)
{
...
long CChargeDongDlg3::GetHTypeCNo()
{
	return m_cmbHCustomer.GetItemData(0);
}

BOOL  CChargeDongDlg3::IsHTypeViewMode()
{
	return m_bHTypeViewMode;
}


void CChargeDongDlg3::OnCancel()
{
...
void CChargeDongDlg3::OnPaint()
{
	CPaintDC dc(this); // device context for painting
	LF->FillBackground(&dc, this); 
}

void CChargeDongDlg3::OnBnClickedTwowayCheck()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}

HBRUSH CChargeDongDlg3::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CChargeDongDlg3::OnBnClickedGeneralRaboCheck()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}


void CChargeDongDlg3::SetRemoveRefreshList()
{
