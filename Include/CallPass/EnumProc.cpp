...
HWND CEnumProc::m_hParentWnd;
PROCESSMAP CEnumProc::m_ProcessMap;
BOOL CEnumProc::m_bWin98 = FALSE;
					...
					if (hProcess != NULL) {
						...
						if (lpfEnumProcessModules(hProcess, &hMod,
							sizeof(hMod), &dwSize2)) {
								...
								// Get the module name
								if (!lpfGetModuleFileName(hProcess, hMod,
...
//						break;
					...
					if(!OnProcess(szFileName, lpdwPIDs[dwIndex]))
						break;
					...
					if (_stricmp(szFileName, "NTVDM.EXE") == 0) {
						...
						sInfo.lpProc = NULL;//lpProc;
						sInfo.lParam = NULL;//(DWORD) lParam;
						sInfo.bEnd = FALSE;
...
//							procentry.szExeFile, lParam)) {
						if(OnProcess(procentry.szExeFile, procentry.th32ProcessID))
						{
								// Did we just bump into an NTVDM?
...
//		psInfo->lParam);
	...
	bRet = CEnumProc::OnProcess(pszFileName, psInfo->dwPID);
