...
static int SP1[BOXLEN];
static int SP2[BOXLEN];
static int SP3[BOXLEN];
static int SP4[BOXLEN];
static int SP5[BOXLEN];
static int SP6[BOXLEN];
static int SP7[BOXLEN];
static int SP8[BOXLEN];
...
static void spbox(MBYTE* box, int* mask, int* compiled)
{
	int i;
...
static void KDspbox(MBYTE *box, int *mask, int *compiled,
    MBYTE *key, int offset, int index)
{
    int i;
	...
	for (i = 0; i < BOXLEN; i++) {
		...
		value = (MBYTE)((xor ^box[spindex]) & 0xF);
...
void initKDdesSPboxes(MBYTE *key, int offset)
{
...
static void DES_SPboxes()
{
    initDesSPboxes();
}

/*
static void DES_SPboxes2(int dummy)
{
    inits3DesSPboxes();
}

static void DES_SPboxes(MBYTE *key, int offset)
{
...
static void key_single( MBYTE *hexkey, int off1, int mode, int* keybuf, int offset)
{
	int bbidx = 0;
	int* bip;
	int* kip;
	...
	while (bbidx < 56) {
		...
		bbidx++;
		// only 7 bits out of each MBYTE dor DES
	}
...
static void key_dbl( MBYTE *hexkey, int off1, int mode, int *keyout, int offset )
{
	int i;
...
static void key_triple( MBYTE *hexkey, int off1, int mode, int *keyout, int offset )
{
	int first = 0;
	int third = 2 * KEYSIZE;
	int revmod;
...
static void engine_single( MBYTE *inblock, int offin, MBYTE* outblock, int offout, int* keys)
{
	//DES_SPboxes key 
	...
	leftt ^= work;
	right ^= (work << 4);
	outblock[0 + offout] = (MBYTE)((right >> 24) & 0xFF);
	outblock[1 + offout] = (MBYTE)((right >> 16) & 0xFF);
	outblock[2 + offout] = (MBYTE)((right >> 8) & 0xFF);
	outblock[3 + offout] = (MBYTE)((right) & 0xFF);
	outblock[4 + offout] = (MBYTE)((leftt >> 24) & 0xFF);
	outblock[5 + offout] = (MBYTE)((leftt >> 16) & 0xFF);
	outblock[6 + offout] = (MBYTE)((leftt >> 8) & 0xFF);
	outblock[7 + offout] = (MBYTE)((leftt) & 0xFF);

	return;
}

static void engine_triple( MBYTE *inblock, int offin, MBYTE *outblock, int offout, int *keys)
{
	//DES_SPboxes key 
...
void des_init(MBYTE *key, int offset, BOOL triple)
{
...
void des_ecb(BOOL encrypt, MBYTE* in, int offin, MBYTE* out, int offout)
{
	if (encrypt) {
